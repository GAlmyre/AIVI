			...
			cv::Mat &compensated)
{
  ...
  // copy the previous frame to the compensated image
  ...
  for (int i = 0; i < blockXCount; i++) {
    for (int j = 0; j < blockYCount; j++) {
      ...
      // get the vector of this block and use it for the displacement
      cv::Vec2i currentVector = motionVectors.at<cv::Vec2i>(j,i);

      int newI = i*blockSize+currentVector(0);
      int newJ = j*blockSize+currentVector(1);
      ...
      if (newI >= prev.cols-blockSize) {
        newI = prev.cols-1-blockSize;
      }
      if (newJ >= prev.rows-blockSize) {
        newJ = prev.rows-1-blockSize;
      }
      cv::Mat newBlock = prev(cv::Rect(newI, newJ, blockSize, blockSize));
      cv::Mat dst_roi = compensated(cv::Rect(i*blockSize, j*blockSize, blockSize, blockSize));
      newBlock.copyTo(dst_roi);
    }
  }
