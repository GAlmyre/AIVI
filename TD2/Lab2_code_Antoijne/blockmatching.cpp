		       ...
		       cv::Mat &motionVectors)
{
  ...
  cv::Mat blockM1;
  blockM1.create(blockSize, blockSize, CV_32SC2);

  cv::Mat blockM2;
  blockM2.create(blockSize, blockSize, CV_32SC2);

  int iBlockBest;
  int jBlockBest;
  cv::Mat blockBest;
  blockBest.create(blockSize, blockSize, CV_32SC2);

  double lowestMSE = DBL_MAX;
  ...
  for(int i = 0 ; i < m1.rows-blockSize-1 ; i+=blockSize){
    for(int j = 0 ; j < m1.cols-blockSize-1 ; j+=blockSize){
      ...
      blockM1 = m1(cv::Rect(j, i, blockSize, blockSize));
      lowestMSE = DBL_MAX;      
      for(int k = i-windowSize/2 ; k < i+windowSize/2-blockSize-1 ; k++){
      	for(int l = j-windowSize/2 ; l < j+windowSize/2-blockSize-1 ; l++){

      	  if(k>=0 && k < m1.rows - blockSize-1 && l >=0 && l < m1.cols - blockSize-1){
	    
      	    blockM2 = m2(cv::Rect(l, k, blockSize, blockSize));
      	    double mse = computeMSE(blockM1, blockM2);
      ...
      // std::cout << "i : " << i << ", iBlockBest : " << iBlockBest << std::endl;
      // std::cout << "j : " << j << ", jBlockBest : " << jBlockBest << std::endl;
      motionVectors.at<cv::Vec2i>(i/blockSize , j/blockSize) = cv::Vec2i(iBlockBest-i, jBlockBest-j);
  ...
  // for(int i = 0 ; i < m1.rows-blockSize-1 ; i+=blockSize){
  //   for(int j = 0 ; j < m1.cols-blockSize-1 ; j+=blockSize){
  //     // std::cout << "DEBUT : i+ blockSize = " << i+blockSize << ", j+blockSize : " << j+blockSize << std::endl;
  ...
  // 	  if(k>=0 && l >= 0 && k < m1.cols - blockSize-1 && l < m1.rows - blockSize-1){
  ...
  //   }
  // }
  // std::cout << " COUCOUCOCUCOUCOU" << std::endl;
