...
void drawMV(cv::Mat &img, const cv::Point &pt1, const cv::Point &pt2, 
...
void computeCompensatedImage(const cv::Mat &motionVectors,
			     const cv::Mat &prev,
			     cv::Mat &compensated) {
  ...
  for(int i = 0 ; i < blockXCount ; i++){
    for(int j = 0 ; j < blockYCount ; j++){
      ...
      cv::Vec2i vector = motionVectors.at<cv::Vec2i>(i, j);
      
      int compensatedI = i*blockSize + vector(0);
      int compensatedJ = j*blockSize + vector(1);
	...
	error = true;
      ...
      if(compensatedI >= prev.cols-blockSize){
	compensatedI = prev.cols-1-blockSize;
	error = true;
      }
      if(compensatedJ >= prev.rows-blockSize){ 
	compensatedJ = prev.rows-1-blockSize;
	error = true;
      }
      
      // if(error)
      // 	std::cout << "ERRORRR " << nbErrors++ << std::endl;
            
      cv::Mat compensatedBlock = prev(cv::Rect(compensatedI, compensatedJ, blockSize, blockSize));
      
      cv::Mat dst_roi = compensated(cv::Rect(i*blockSize, j*blockSize, blockSize, blockSize));
      compensatedBlock.copyTo(dst_roi);
